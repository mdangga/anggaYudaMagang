...
const columnDefs = ref([
    ...
    },
    {
        ...
        cellRenderer: (params) => {
            ...
            editBtn.onclick = () => router.visit(route('category.edit', params.data.id_category))
            ...
            deleteBtn.innerHTML = `
                ...
                Hapus
            `
            deleteBtn.onclick = () => openDeleteModal(params.data.id_category, params.data.name_category)
...
const fetchCategories = async () => {
    try {
        const { data } = await axios.get('/categories/ajax')
...
const selectedCategory = computed(() => {
    ...
    return props.category.data.find(c => c.id_category === itemIdToDelete.value)
})

const warningMessage = computed(() => {
    if (!selectedCategory.value) return 'Data yang dihapus tidak dapat dikembalikan.'
    if (selectedCategory.value.locations_count > 0) {
        return `Kategori ini memiliki ${selectedCategory.value.location_count} lokasi. Menghapus kategori akan mempengaruhi data terkait.`
    }
    return 'Data yang dihapus tidak dapat dikembalikan.'
})

const deleteMessage = computed(() => {
    return `Apakah Anda yakin ingin menghapus kategori "${itemName.value}"?`
})
...
const deleteItem = () => {
    ...
    router.delete(route('category.destroy', itemIdToDelete.value), {
        preserveScroll: true,
        onSuccess: () => {
            toast.success("Kategori berhasil dihapus !", { position: "top-right", autoClose: 3000 });
            showDeleteModal.value = false
            itemIdToDelete.value = null
            itemName.value = ''
        },
...
<template>
    <Head :title="`Kategori - ${$page.props.profile_web.app_name}`" />
        ...
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <Link :href="route('category.create')"
                    ...
                    <span class="relative text-sm">Tambah Kategori</span>
                </Link>
            </div>
        </template>
